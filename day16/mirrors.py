    ...
    def __next__(self):
        ...
        for ray,last in enumerate(last_ray):
            ...
            if len(delta)==1:
                newdir = delta.pop()
                dx, dy = MOVE[newdir]
                if last.x + dx >= 0 and last.x + dx < self.cols and last.y + dy >= 0 and last.y+dy < self.rows and (last.x,last.y,newdir) not in self.seen:
                    self.beam[ray].append(Beam(last.x+dx,last.y+dy,1,newdir))
                    self.seen.add((last.x,last.y,newdir))
                else:
